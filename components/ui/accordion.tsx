...
>(({ className, children, ...props }, ref) => (
  <AccordionPrimitive.Header className="flex">
    <AccordionPrimitive.Trigger
      ref={ref}
      className={cn(
        "flex flex-1 items-center justify-between py-5 text-left font-serif text-xl text-ink transition-all hover:text-moss [&[data-state=open]>svg]:rotate-180",
        className
      )}
      {...props}
    >
...
>(({ className, children, ...props }, ref) => (
  <AccordionPrimitive.Content
    ...
    {...props}
  >
    <div className={cn("pb-5 pt-0 max-w-[60ch]", className)}>{children}</div>
  </AccordionPrimitive.Content>
));
